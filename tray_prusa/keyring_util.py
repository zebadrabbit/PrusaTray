...
try:
    ...
    logger.warning(
        "keyring library not available - credentials will only be available via environment variables"
    )
...
def get_secret(key: str) -> Optional[str]:
    ...
    if secret:
        logger.info(
            f"Retrieved secret '{key}' from environment variable {env_var_name}"
        )
        return secret
...
def set_secret(key: str, value: str) -> bool:
    ...
    if not KEYRING_AVAILABLE:
        ...
        logger.info(
            f"To use this secret, set environment variable: PRUSATRAY_PASSWORD_{_sanitize_key_for_env(key)}"
        )
        return False
