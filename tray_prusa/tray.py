...
class PrusaTrayIcon(QObject):
    ...
    def show_message(self, title: str, message: str) -> None:
        ...
        self.tray_icon.showMessage(
            title, message, QSystemTrayIcon.MessageIcon.Information, 3000  # 3 seconds
        )
            ...
            if timer.isActive():
                ...
                if error == QNetworkReply.NetworkError.NoError and status_code == 200:
                    ...
                    self.test_completed.emit(
                        False, f"Authentication failed (HTTP {status_code})"
                    )
                    ...
                    self.test_completed.emit(
                        False, f"Unexpected status code: {status_code}"
                    )
            else:
                reply.abort()
...
class CredentialsDialog(QDialog):
    ...
    def __init__(self, config_manager: ConfigManager, parent: Optional[QWidget] = None):
        ...
        if config.printer_base_url and config.username:
            existing_pw = keyring_util.get_password(
    ...
    def _test_connection(self) -> None:
        ...
        if not ConfigManager.validate_url(base_url):
            QMessageBox.warning(
                self, "Invalid URL", "Please enter a valid HTTP or HTTPS URL"
            )
            return

        if auth_mode != "none" and (not username or not password):
            QMessageBox.warning(
                self, "Invalid Input", "Please enter username and password/API key"
            )
            return
    ...
    def accept(self) -> None:
        ...
        try:
            self.config_manager.save(new_config)
            logger.info(
                f"Saved credentials: URL={base_url}, username={username}, auth_mode={auth_mode}"
            )
